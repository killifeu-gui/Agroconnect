...
def init_db():
    ...
    if True:
        ...
        produits_data = [
            ...
            {"nom":"Carottes orange", "vendeur_id": all_users[0]["_id"], "vendeur_pseudo": all_users[0]["pseudo"], "vendeur_avatar": all_users[0]["avatar"], "categorie": "Légumes", "prix": 400, "image": "https://images.unsplash.com/photo-1584588694380-4d71bcdd2160?w=400&h=300&fit=crop", "description": "Carottes fraîches pleines de vitamines", "stock": 60, "disponible": True, "region": all_users[0]["region"], "date_ajout": datetime.now(), "note_moyenne": 4.7, "nb_avis": 10},
            {"nom":"Pastèques sucrées", "vendeur_id": all_users[1]["_id"], "vendeur_pseudo": all_users[1]["pseudo"], "vendeur_avatar": all_users[1]["avatar"], "categorie": "Fruits", "prix": 2000, "image": "https://images.unsplash.com/photo-1590981268773-a7bb4f61c8e3?w=400&h=300&fit=crop", "description": "Pastèques juteuses et sucrées", "stock": 20, "disponible": True, "region": all_users[1]["region"], "date_ajout": datetime.now(), "note_moyenne": 4.8, "nb_avis": 12},
            {"nom":"Mangues Ataulfo", "vendeur_id": all_users[0]["_id"], "vendeur_pseudo": all_users[0]["pseudo"], "vendeur_avatar": all_users[0]["avatar"], "categorie": "Fruits", "prix": 1500, "image": "https://images.unsplash.com/photo-1585518419759-37f4ea6a66e3?w=400&h=300&fit=crop", "description": "Mangues de saison au goût exceptionnel", "stock": 35, "disponible": True, "region": all_users[0]["region"], "date_ajout": datetime.now(), "note_moyenne": 4.9, "nb_avis": 15},
            {"nom":"Papayes", "vendeur_id": all_users[1]["_id"], "vendeur_pseudo": all_users[1]["pseudo"], "vendeur_avatar": all_users[1]["avatar"], "categorie": "Fruits", "prix": 1200, "image": "https://images.unsplash.com/photo-1590080876004-13a4fcf22d4e?w=400&h=300&fit=crop", "description": "Papayes jaunes bien mûres", "stock": 40, "disponible": True, "region": all_users[1]["region"], "date_ajout": datetime.now(), "note_moyenne": 4.3, "nb_avis": 7},
            {"nom":"Laitue fraîche", "vendeur_id": all_users[0]["_id"], "vendeur_pseudo": all_users[0]["pseudo"], "vendeur_avatar": all_users[0]["avatar"], "categorie": "Légumes", "prix": 350, "image": "https://images.unsplash.com/photo-1516895921230-8ac86ef6e7b0?w=400&h=300&fit=crop", "description": "Laitue verte tendre", "stock": 80, "disponible": True, "region": all_users[0]["region"], "date_ajout": datetime.now(), "note_moyenne": 4.4, "nb_avis": 9},
            {"nom":"Poivrons rouges", "vendeur_id": all_users[1]["_id"], "vendeur_pseudo": all_users[1]["pseudo"], "vendeur_avatar": all_users[1]["avatar"], "categorie": "Légumes", "prix": 600, "image": "https://images.unsplash.com/photo-1599599811035-8dd37388c328?w=400&h=300&fit=crop", "description": "Poivrons rouges juteux et savoureux", "stock": 45, "disponible": True, "region": all_users[1]["region"], "date_ajout": datetime.now(), "note_moyenne": 4.6, "nb_avis": 11},
            {"nom":"Agrumes mélangés", "vendeur_id": all_users[0]["_id"], "vendeur_pseudo": all_users[0]["pseudo"], "vendeur_avatar": all_users[0]["avatar"], "categorie": "Fruits", "prix": 1800, "image": "https://images.unsplash.com/photo-1559827260-dc66d52bef19?w=400&h=300&fit=crop", "description": "Oranges, citrons, pamplemousses frais", "stock": 100, "disponible": True, "region": all_users[0]["region"], "date_ajout": datetime.now(), "note_moyenne": 4.5, "nb_avis": 8},
            {"nom":"Aubergines", "vendeur_id": all_users[1]["_id"], "vendeur_pseudo": all_users[1]["pseudo"], "vendeur_avatar": all_users[1]["avatar"], "categorie": "Légumes", "prix": 550, "image": "https://images.unsplash.com/photo-1520072959219-c595dc870360?w=400&h=300&fit=crop", "description": "Aubergines violettes de qualité", "stock": 55, "disponible": True, "region": all_users[1]["region"], "date_ajout": datetime.now(), "note_moyenne": 4.1, "nb_avis": 5},
        ]
...
def login():
    ...
    if request.method == "POST":
        ...
        print(f"🔐 Login attempt: {pseudo}")
        
        if not pseudo or not password:
            print("❌ Missing pseudo or password")
            return render_template("login.html", error="Pseudo et mot de passe requis")
        
        user = utilisateurs.find_one({"pseudo": pseudo})
        if not user:
            print(f"❌ User not found: {pseudo}")
            return render_template("login.html", error="Identifiants incorrects")
        
        if not check_password_hash(user["password_hash"], password):
            print(f"❌ Wrong password for {pseudo}")
            return render_template("login.html", error="Identifiants incorrects")
        
        session["user_id"] = str(user["_id"])
        session["pseudo"] = user["pseudo"]
        session["role"] = user.get("role", "user")
        print(f"✅ Login successful: {pseudo}")
